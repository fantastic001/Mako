...
LOG_FILE="logs/project_subprojects_add.log"
do_test() 
{
	set -x 
	...
	mako project "test" subprojects add subtest2
	set +x
}

check_success() 
{
	mako project test subprojects | grep subtest2
	return $?
}

log_error() 
{
	echo "TEST FAILED: project_subprojects_add" >> $LOG_FILE
...
echo "Executing test project_subprojects_add"
echo > $LOG_FILE
