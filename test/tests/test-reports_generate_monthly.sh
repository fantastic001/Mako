...
LOG_FILE="logs/reports_generate_monthly.log"
do_test() 
{
	set -x 
...
check_success() 
{
	mako reports | grep "Monthly report"
	return $?
}

log_error() 
{
	echo "TEST FAILED: reports_generate_monthly" >> $LOG_FILE
...
echo "Executing test reports_generate_monthly"
echo > $LOG_FILE
