...
LOG_FILE="logs/project_subprojects_delete.log"
do_test() 
{
	set -x 
	...
	mako project "test" subprojects delete subtest 
	set +x
}
...
log_error() 
{
	echo "TEST FAILED: project_subprojects_delete" >> $LOG_FILE
...
echo "Executing test project_subprojects_delete"
echo > $LOG_FILE
